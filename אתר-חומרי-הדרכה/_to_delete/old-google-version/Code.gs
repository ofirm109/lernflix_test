 ...
 *  עם קישור להורדת תבנית האפיון.
 *
 *  אין קובץ מצורף - הקישור מוריד את הקובץ ישירות מ-GitHub Pages.
 *  זה עוקף מסנני ספאם שחוסמים צרופות, ומקל על המייל.
 *
...
const SENDER_NAME = 'אופיר מזרחי';

// כתובת לתשובות (Reply-To)
const REPLY_TO = 'ofir.mi@mevaker.gov.il';

// כתובת ההורדה של תבנית האפיון
const DOC_URL = 'https://ofirm109.github.io/lernflix_test/%D7%AA%D7%91%D7%A0%D7%99%D7%AA_%D7%90%D7%99%D7%A4%D7%99%D7%95%D7%9F.docx';


/* ======================= סוף הגדרות ======================= */


// קידומת למפתחות התור הממתין
...
function doPost(e) {
  try {
    ...
    if (e && e.postData && e.postData.contents) {
      try {
        email = String(JSON.parse(e.postData.contents).email || '').trim();
    ...
    // אם המכסה היומית נגמרה - שומרים בתור ונשלח מחר
    ...
    try {
      sendMaterials_(email);
    } catch (sendErr) {
      // המכסה עשויה הייתה להיגמר בדיוק עכשיו - לא מאבדים את הפונה
      console.error('send failed, queued instead: ' + sendErr);
      queueRequest_(email);
      cache.put(key, '1', 60);
      return json({ ok: true, queued: true });
    }
...
/** שמירת פונה בתור, עד שהמכסה היומית תתאפס. */
function queueRequest_(email) {
  PropertiesService.getScriptProperties()
    .setProperty(QUEUE_PREFIX + email.toLowerCase(), new Date().toISOString());
}


/**
 * שליחת כל מי שממתין בתור.
 * מופעל אוטומטית על ידי טריגר יומי.
 * מה שלא נכנס במכסה של היום נשאר בתור למחרתיים.
 */
function flushQueue() {
  var props = PropertiesService.getScriptProperties();
  var all = props.getProperties();
  var keys = Object.keys(all).filter(function (k) { return k.indexOf(QUEUE_PREFIX) === 0; });
  ...
  console.log('בתור: ' + keys.length + ' | מכסה פנויה: ' + quota);
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var email = keys[i].substring(QUEUE_PREFIX.length);
    ...
    try {
      ...
      console.error('שליחה נכשלה עבור ' + email + ': ' + err);
      break;
    }
  }

  console.log('נשלחו: ' + sent + ' | נותרו בתור: ' + (keys.length - sent));
}


/** כמה ממתינים בתור כרגע - לבדיקה ידנית. */
function showQueue() {
  var all = PropertiesService.getScriptProperties().getProperties();
  var list = Object.keys(all)
    .filter(function (k) { return k.indexOf(QUEUE_PREFIX) === 0; })
    .map(function (k) { return k.substring(QUEUE_PREFIX.length); });
  console.log('ממתינים בתור (' + list.length + '): ' + (list.join(', ') || '—'));
  console.log('מכסה שנותרה היום: ' + MailApp.getRemainingDailyQuota());
}
...
const PLAIN_TEXT =
  ...
  '2. תבנית אפיון - תבנית עבודה מסודרת לאפיון של מערכות וכלים חדשים באמצעות בינה מלאכותית (AI):\n' +
  DOC_URL + '\n\n' +
    ...
    <tr>
      <td style="padding:0; font-size:0; line-height:0;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;">
          <tr>
            <td width="60%" style="width:60%; height:5px; background-color:#16305a; line-height:5px; font-size:0;">&nbsp;</td>
            <td width="40%" style="width:40%; height:5px; background-color:#f0b429; line-height:5px; font-size:0;">&nbsp;</td>
          </tr>
        </table>
      </td>
    </tr>
