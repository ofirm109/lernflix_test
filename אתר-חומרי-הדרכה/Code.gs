...
function queueRequest_(email, base) {
  PropertiesService.getScriptProperties()
    .setProperty(QUEUE_PREFIX + email.toLowerCase(), base || '');
}
...
function flushQueue() {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var attachment = getAttachment_(all[keys[i]]);

    if (!attachment) {
      console.error('הקובץ המצורף לא זמין - עוצרים, התור נשמר.');
      break;
    }
